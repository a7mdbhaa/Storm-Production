...
let stopping = false;
async function shutdown(signal) { if (stopping)
    return; stopping = true; logger.info({ signal }, 'Graceful shutdown'); client.destroy(); await prisma.$disconnect(); process.exit(0); }
process.on('SIGINT', () => void shutdown('SIGINT'));
process.on('SIGTERM', () => void shutdown('SIGTERM'));
process.on('unhandledRejection', e => logger.error({ error: e instanceof Error ? e.message : String(e) }, 'Unhandled rejection'));
process.on('uncaughtException', e => logger.fatal({ error: e.message }, 'Uncaught exception'));
